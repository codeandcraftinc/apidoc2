...
var defaults = {
    dest: path.join(process.cwd(), 'doc/'),
...
function createDoc(options) {
    ...
    if (options.lineEnding) {
        ...
        else if (options.lineEnding === 'CR') options.lineEnding = '\r'; // darwin
    ...
    try {
        ...
        var json = JSON.parse(fs.readFileSync(path.join(apidocPath, 'package.json'), 'utf8'));
