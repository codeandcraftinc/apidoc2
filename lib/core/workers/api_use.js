        ...
        parsedFile.forEach(function (block) {
            ...
            block.local[target].forEach(function (definition) {
                ...
                if (!preProcess[source] || !preProcess[source][name]) {
                    throw new WorkerError('Referenced groupname does not exist / it is not defined with @apiDefine.', filenames[parsedFileIndex], block.index, messages.common.element, messages.common.usage, messages.common.example, [{ 'Groupname': name }]);
                }
                ...
                if (preProcess[source][name][version]) {
                    ...
                    versionKeys.forEach(function (currentVersion, versionIndex) {
                        if (versionsUtil.gte(version, currentVersion) && versionsUtil.gte(currentVersion, lastVersion)) {
                    ...
                    if (foundIndex === -1) {
                        throw new WorkerError('Referenced definition has no matching or a higher version. ' + 'Check version number in referenced define block.', filenames[parsedFileIndex], block.index, messages.common.element, messages.common.usage, messages.common.example, [{ 'Groupname': name }, { 'Version': version }, { 'Defined versions': versionKeys }]);
                    }
