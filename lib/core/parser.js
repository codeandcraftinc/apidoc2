    ...
    for (var i = 0; i < indexApiBlocks.length; i += 1) {
        ...
        for (var j = 0; j < elements.length; j += 1) {
            ...
            if (!elementParser) {
                ...
                if (!values) throw new ParserError('Empty parser result.', self.filename, blockIndex + 1, element.sourceName, element.source);
                ...
                if (preventGlobal) {
                    ...
                    if (Object.keys(blockData.global).length > countAllowedMultiple) throw new ParserError('Only one definition or usage is allowed in the same block.', self.filename, blockIndex + 1, element.sourceName, element.source);
                }
                ...
                if (pathTo === 'global' || pathTo.substr(0, 7) === 'global.') {
                    if (allowMultiple) {
                        ...
                        if (Object.keys(blockData.global).length > 0) throw new ParserError('Only one definition is allowed in the same block.', self.filename, blockIndex + 1, element.sourceName, element.source);

                        if (preventGlobal === true) throw new ParserError('Only one definition or usage is allowed in the same block.', self.filename, blockIndex + 1, element.sourceName, element.source);
                    }
                }
